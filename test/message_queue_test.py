...
def test_put_message(node_with_test_topic):
    assert node_with_test_topic.put_message(TEST_TOPIC, TEST_MESSAGE).json() == {
        "success": True
    }
...
def test_put_and_get_message(node_with_test_topic):
    ...
    assert node_with_test_topic.get_message(TEST_TOPIC).json() == {
        "success": True,
...
def test_put2_and_get1_message(node_with_test_topic):
    ...
    assert node_with_test_topic.put_message(TEST_TOPIC, second_message).json() == {
        "success": True
    }
    assert node_with_test_topic.get_message(TEST_TOPIC).json() == {
        "success": True,
